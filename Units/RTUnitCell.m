...
- (void)applyLayoutAttributes:(RTUnitLayoutAttributes *)layoutAttributes {
	...
	if (!self.leftAligned) NSLog(@"%s : frame=%@", __FUNCTION__, NSStringFromCGRect(layoutAttributes.frame));
...
//	if (!self.leftAligned) NSLog(@"%s : pre-super: %@", __FUNCTION__, NSStringFromCGRect(self.frame));
